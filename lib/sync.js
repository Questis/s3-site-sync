...
function Sync(config) {
	...
	this.cloudfront = new this.AWS.CloudFront({apiVersion: '2017-10-30'});
	...
	return BPromise.coroutine(function*() {
		...
		if (self.cloudfrontDistribution) {
			let invalidationRes = yield self.cloudfrontFn('createInvalidation', {
				...
				InvalidationBatch: {
					CallerReference: nanoId('1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', 8),
					Paths: {
						Quantity: 1,
						Items:    ['/*']
					}
...
Sync.prototype.ensureBucketExists = function () {
	...
	return this.s3Fn('listBuckets').then(function (response) {
		console.log('All buckets:\n', _.map(response.data.Buckets, 'Name'));
...
Sync.prototype.ensureBucketWebsite = function () {
	...
	return this.s3Fn('getBucketWebsite', {Bucket: config.s3Options.Bucket}).then(function (response) {
		...
		if (config.noBucketWebsiteRoutingRules) {
			// A "better" way to do this is to set up an "error page" handler in CloudFront. This should point to `/index.html` (or whatever) in the
			// case of a "404" error, and return a custom response code of 200
			bucketWebsite.RoutingRules = null;
		} else if (bucketWebsite.RoutingRules && bucketWebsite.RoutingRules[0] && !bucketWebsite.RoutingRules[0].Redirect.HostName) {
...
Sync.prototype.createDistribution = function () {
	...
	distributionConfig.CallerReference = nanoId('1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', 8);
	...
	if (self.config.s3Options.Prefix) {
...
Sync.prototype.ensureDistributionDefaultRootObj = function () {
	...
	return self.cloudfrontFn('getDistribution', {Id: self.cloudfrontDistribution.Id}).then(function (response) {
		...
		// OriginPath must start with slash, and not have trailing slash
		if (self.config.s3Options.Prefix) {
			distribution.DistributionConfig.Origins.Items[0].OriginPath = !self.config.s3Options.Prefix ? '' :
			'/' + bucketPrefix.replace(/^(\\|\/)/, '').replace(/(\\\/)*$/, '');
		}
